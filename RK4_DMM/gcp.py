...
def rhs(rho, h, inv_ovlp, identity, mu):
    ...
    """
    scaledH = -0.5 * (inv_ovlp @ h - mu * identity)
    k = (identity - inv_ovlp @ rho) @ scaledH
    f = rho @ k + k.conj().T @ rho
    return f
...
def single_step(rho_, kwargs):
    ...
    """
    # sort kwargs into variables
    h1e = kwargs['h1e']
    mf = kwargs['mf']
    dbeta = kwargs['dbeta']
    rk4steps = kwargs['rk4steps']
    inv_ovlp = kwargs['inv_ovlp']
    mu = kwargs['mu']
