...
colors = np.arange(len(scipy_energies))
plt.figure(1)
plt.subplot(111)
plt.ylim([-0.1, 0.5])
plt.xlabel("Energy")
plt.ylabel("Norm")
#plt.scatter(scipy_energies, scipy_norms, label="scipy", c=colors, marker='o')
indx = np.argsort(zvode_energies)
plt.scatter(zvode_energies[indx], zvode_norms[indx], label="zvode", c=colors, marker='v')
plt.scatter(ntpoly_energies[indx], ntpoly_norms[indx], label="ntpoly", c=colors, marker='o', alpha=0.7, edgecolor='k')
plt.legend()
